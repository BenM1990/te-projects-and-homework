...
--Write a query to retrieve the names of captial cities in each state
--SELECT * FROM state;
--SELECT * FROM city;
...
--Where does the park name live? ->
--Where deos state abbreviation live? -> park_state or state
...
JOIN state ON park_state.state_abbreviation = state.state_abbreviation; -- join the state table to park_ state where states match
...
-- Write a query to retrieve the state name and the earliest date a park was established in that state 
--(or territory) for every record in the state table that has park records associated with it.

SELECT state_name, MIN(date_established) AS easliest_date
FROM state 
...
--keywords above: "FOR EVERY RECORD IN THE STATE TABLE" -- probably means a LEFT JOIN
...
UNION 
SELECT state_name, 'State' AS typle_of_place FROM state
WHERE state_name LIKE 'W%';
