...
--VALUES (64, 'CA') --using my human eyeballs to go to the park_id is not good practice
...
INSERT INTO city (city_name, state_abbreviation, population, area) VALUES ('Kevanaheim', 'XX', 1, 0.1);
...
--'CA' is a primary key on the state table, and is used as a foreign key in other table, so deleting the record from the state table
...
COMMIT; -- commiting a transaction makes the changes permanentyl applied to the database
...
SELECT * FROM city WHERE city_name = 'Schrodiners City';

SELECT * FROM city WITH(NOLOCK) WHERE city_name = 'Schrodingers City';
