...
-- The names from all the records (rows of data) in the state table
--SELECT * FROM state; -- this one selects ALL the rows and columns from the state table
SELECT state_name FROM state;


-- The names and populations of all cities
--SELECT * FROM city;
SELECT city_name, population FROM city; -- seperate column with commas to select multiple columns
...
--SELECT city_name, state_abbreviation FROM city WHERE state_abbreviation <> 'CA'; -- <> other way to say !=
...
SELECT * FROM state;
SELECT state_name FROM state WHERE census_region IS NULL; -- not the NULL keyword;
...
--how do I turn square km into sg meters? -> multiply by 1000000
SELECT * FROM park;
